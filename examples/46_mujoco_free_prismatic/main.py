...
# Body 1 at (0, 1,  0.4): prismatic along Z axis
# body1 frame = interface between the two cubes; geom offset places mass center at (0, 1, 0.4).
MJCF_MODEL = """
<mujoco>
  ...
  <worldbody>
    <body name="body0" pos="0 1 -0.4">
      ...
      <body name="body1" pos="0 0 0.4">
        <joint name="prismatic" type="slide" axis="0 0 1"/>
...
INIT_PRISMATIC_VEL = 1.0  # m/s along slide axis
mj_data.qvel[6] = INIT_PRISMATIC_VEL
...
config["newton"]["velocity_tol"] = 1e-3
config["newton"]["transrate_tol"] = 1e-3
...
def update_ref_dof_prev(info: Animation.UpdateInfo):
    ...
    for i in range(N_BODIES):
        rdp_view[i] = affine_body.transform_to_q(mj_state["transforms"][i]).reshape(-1, 1)


scene.animator().insert(body_object, update_ref_dof_prev)

# ------------------------------------------------------------------
# Joints: FreeJoint + Prismatic
# ------------------------------------------------------------------
...
# Prismatic joint: instance 0 -> instance 1 (axis along +Z)
# Anchor at (0, 1, 0) = interface between the two cubes, matching MuJoCo slide joint.
abpj = AffineBodyPrismaticJoint()
joint_anchor = np.array([0.0, 1.0, 0.0], dtype=np.float32)
slide_dir = np.array([0.0, 0.0, 1.0], dtype=np.float32)
pris_pos0 = np.array([joint_anchor], dtype=np.float32)
pris_pos1 = np.array([joint_anchor + slide_dir], dtype=np.float32)
prismatic_mesh = abpj.create_geometry(
    pris_pos0, pris_pos1, [geo_slot], [0], [geo_slot], [1], [100.0]
)
prismatic_object = scene.objects().create("prismatic_joint")
prismatic_slot, _ = prismatic_object.geometries().create(prismatic_mesh)

# ------------------------------------------------------------------
# ExternalArticulationConstraint: 6 free + 1 prismatic = 7 DOFs
# ------------------------------------------------------------------
eac = ExternalArticulationConstraint()

joint_geos = [free_joint_slot] * N_FREE + [prismatic_slot]
...
def on_update():
    ...
    imgui.Text("Body 0 (free) -> Body 1 (prismatic Z)")
    ...
    imgui.Text(f"  prismatic:  {M[6,6]:.4f}")
    ...
    if gui["mujoco_only"]:
        ...
        # delta_theta_tilde from actual qpos displacement (not qvel*dt)
